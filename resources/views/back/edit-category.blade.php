...
@endif
        ...
        <form action="{{ route('category.update', $category->slug) }}" method="POST" enctype="multipart/form-data">
            ...
            <div>
                <label>Title</label>
                <input type="text" id="name" placeholder="Title" name="name" value="{{$category->name}}" onkeyup="generateSlug()" required>
            </div>
            <div class="row mb-4" style="align-items: center;">
                <label for="horizontal-firstname-input" class="col-form-label">Meta Tag</label>
                <input id="slug" name="slug" type="text" class="form-control" value="{{$category->slug}}" placeholder="Add meta tag" />
            </div>
            <div class="row mb-4" style="align-items: center;">
                <label for="horizontal-firstname-input" class="col-form-label" for="image">Add Icon</label>
               <img src="{{ asset('storage/' . $category->img) }}" alt="{{ $category->name }}" width="10%">
